...
- FRAMING — ZERO-SHOT-ONLY IS NOW PRIMARY; POOLED PROBABILITY CONTRASTS ARE WITHDRAWN (`06_stats_results.json` -> `framing_zeroshot_only`, and the diagnostic at `framing/instrument_mixing_diagnostic`). 51.9% of frame rows (6,652 of 12,827) come from the keyword fallback (`frame_method` column), and the two instruments are NOT on a common scale: the zero-shot classifier returns multi-label probabilities, the fallback returns the fraction of dictionary patterns matched, and levels differ ~30x (mean frame_human_interest 0.830 Prog / 0.904 Trad zero-shot vs 0.0256 / 0.0261 keyword). Instrument assignment is correlated with group — 50.2% of progressive-attributed articles are zero-shot-scored vs 45.9% of traditional-attributed — so progressives are disproportionately measured on the high-scale instrument and their pooled means are inflated for reasons unrelated to content.
  - PRIMARY (6,175 model-classified articles): dominant-frame chi-square = 492.10, dof = 4, p = 3.4e-105, Cramers V = 0.282. Rates: accountability 40.4% vs 18.7% (h = 0.48), conflict 37.0% vs 47.7% (h = -0.22), human interest 17.4% vs 32.3% (h = -0.35), reform 5.0% vs 1.2% (h = 0.23), consequences 0.2% vs 0.1% (h = 0.02). Probability-score d: reform +0.607, accountability +0.249, consequences +0.216, human interest -0.413, conflict -0.119; all five BH-robust and all five aligned in sign with the rates.
  - SECONDARY (pooled, 12,827): dominant-frame chi-square = 395.66, dof = 4, Cramers V = 0.214. Reported as secondary, not discarded, because argmax within an article is at least in-kind across instruments — but the fallback's argmax runs over near-zero values and is correspondingly noisy.
  - WITHDRAWN: the pooled frame-PROBABILITY contrasts (accountability +0.133, reform +0.269, consequences +0.174, conflict +0.074, human interest +0.001). PROOF THEY ARE AN ARTIFACT, not merely noisy: for conflict and human interest the pooled d lies **OUTSIDE the range spanned by both subgroups** — conflict pooled +0.074 against zero-shot -0.119 and keyword +0.035; human interest pooled +0.001 against zero-shot -0.413 and keyword -0.008. A genuine weighted combination of two subgroups on a common scale cannot fall outside their range. Per-frame pooled / zero-shot / keyword d: accountability +0.133 / +0.249 / +0.074; conflict +0.074 / -0.119 / +0.035; consequences +0.174 / +0.216 / +0.046; human interest +0.001 / -0.413 / -0.008; reform +0.269 / +0.607 / +0.045.
  - CORRECTED CLAIM: the paper previously explained the pooled-vs-zero-shot gap as the fallback "attenuating" the differential and concluded the headline framing results were CONSERVATIVE. That was wrong — attenuation can only pull an estimate toward zero, never push it outside the range of its parts. The honest statement is that the pooled probability figures are uninterpretable and the zero-shot-only figures, which are LARGER on four of five frames, are the ones to read. The pooled figures are now reported once, in a labelled methodological passage with the out-of-range table, and carry no substantive claim.
  - Knock-on: the stance-frame correlation check is now reported on the zero-shot subset (r = -0.39 accountability to +0.03 human interest, n = 6,157; pooled version -0.21 to -0.10, n = 9,905, kept for completeness). The Figure 5 framing heatmap pools both instruments and its caption now says so.
