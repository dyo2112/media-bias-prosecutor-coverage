...
\begin{figure}[H]
  ...
  \caption{Effect size forest plot with 95\% bootstrap confidence intervals. The green band marks the negligible effect zone ($|d| < 0.2$). Results are grouped by analysis type: overall, paired-county, per-method, and theme attribution. Theme attribution ($d = -0.42$) is the largest raw effect in the study, but it is substantially event-driven and reverses sign once recall and crime-rising themes are removed (Section~\ref{sec:themes}); the plotted magnitude should be read as an upper bound.}
  \label{fig:forest}
\end{figure}
...
Fifth, the theme differential is substantially event-driven and does not survive as a differential-treatment finding. The prosecutor-attributed theme effect is the largest raw effect in the paper ($d = +0.418$), but two themes carry most of it, and both track events that actually differed between the groups rather than differential handling of comparable situations. Excluding articles flagged with the recall theme leaves $d = +0.096$; excluding crime-rising leaves $d = +0.300$; excluding either leaves $d = -0.088$, a sign reversal (Section~\ref{sec:themes}). Both progressive prosecutors in the study faced real recall campaigns and no traditional prosecutor did, so the recall component is not evidence of differential treatment at all, and the crime-rising component reflects a genuine contemporaneous public debate over crime trends. The portion of the theme differential that is not attributable to these two themes is small and slightly favors the traditional group. The theme layer should therefore be read as evidence about \emph{what} progressive-prosecutor coverage was about during these years, not as an estimate of how comparable officials are framed.

Sixth, the pooled cross-sectional composite depends on the composition of the traditional baseline. Across the five leave-one-prosecutor-out fits the composite ranges from $d = -0.233$ to $d = -0.070$ (Appendix~\ref{app:baseline_sensitivity}). The low end is the fit that drops Steve Wagstaffe, whose coverage is the most favorable in the corpus and who is the only prosecutor from San Mateo County, the only county with no transition; his removal halves the pooled effect. The pooled contrast is thus partly a between-county comparison anchored on a single untreated county, and that county is also the comparison series in the controlled segmented ITS. The sign of the contrast is robust across all five fits, but its magnitude is not, and $d \approx -0.07$---the value at which the two within-county pairs and the drop-Wagstaffe fit agree---is the better estimate of the composite differential.
