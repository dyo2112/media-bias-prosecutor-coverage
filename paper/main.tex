...
\midrule
Chesa Boudin     & San Francisco & Progressive  & 2020--2022    & 5,759 \\
Brooke Jenkins   & San Francisco & Traditional  & 2022--present & 3,200 \\
Nancy O'Malley   & Alameda       & Traditional  & 2009--2023    & 1,478 \\
...
\emph{Interrupted time series.} At the San Francisco transition (Boudin to Jenkins, July 2022), no method shows a substantively significant shift: stance ($d = -0.049$, $p = .042$), keywords ($d = 0.036$, $p = .092$), sentiment ($d = 0.045$, $p = .058$), and document ($d = 0.049$, $p = .024$) all produce negligible effects. The general negativity of SF DA coverage persists regardless of who holds office. At the Alameda transition (O'Malley to Price, January 2023), however, the per-method pattern is stark: stance ($d = 0.431$, $p < .001$) and keywords ($d = 0.357$, $p < .001$) show large shifts toward more critical coverage of Price, while sentiment ($d = 0.036$, $p = .468$) and document tone ($d = -0.032$, $p = .456$) show no change whatsoever. The evaluative dimensions respond to the prosecutor transition; the tonal dimensions do not.
...
I use Google's langextract library, which leverages Gemini 2.5 Flash to extract structured information from text with source grounding: every extraction is anchored to a specific span in the original article. The model was prompted with a detailed extraction schema and three diverse few-shot examples (one critical of a progressive prosecutor, one neutral about a traditional prosecutor, one with mixed coverage) to calibrate extraction behavior across ideological contexts. I applied the extraction pipeline to 4,763 prosecutor-attributed articles (3,423 progressive, 1,340 traditional), covering all five prosecutors. The extraction was conducted in batches due to API rate limits; articles were selected in prosecutor-stratified order (processing each prosecutor's full set sequentially) until API quotas were exhausted, yielding extraction rates of approximately 52\% for the two progressive prosecutors (Boudin 2,987/5,773; Price 436/828) and 21\% for the three traditional prosecutors (Jenkins 685/3,207; O'Malley 302/1,497; Wagstaffe 353/1,648). Because extraction rates differ across prosecutors, all comparisons below use per-article rates rather than raw counts.
...
The extraction schema defines five content categories: (1)~\emph{claims against the prosecutor}, classified by type (performance, policy, character, competence) and evidence quality; (2)~\emph{source attributions}, identifying the speaker's role and stance toward the prosecutor; (3)~\emph{causal claims} linking the prosecutor's actions to outcomes; (4)~\emph{policy actions} attributed to the prosecutor; and (5)~\emph{comparisons} between the current prosecutor and predecessors or standards. The pipeline produced 48,642 total extraction instances across the 4,763 articles (mean $= 10.2$ per article).

\subsection*{Results}

\textbf{Source type distribution.} The types of sources quoted differ significantly between prosecutor groups ($\chi^2 = 360.2$, $p < 10^{-72}$, $df = 8$; see Figure~\ref{fig:sourcetypes}). Because the extraction sample is imbalanced by design (3,423 progressive vs.\ 1,340 traditional articles), all comparisons are expressed as proportions of total sources within each group and as per-article rates:

\begin{table}[ht]
\centering
\caption{Source type distribution by prosecutor ideology. Proportions are within-group; rates are per attributed article (progressive $n = 3{,}423$; traditional $n = 1{,}340$).}
...
\midrule
Police             & 11.9\% & 12.6\% & 0.74 & 0.66 & $1.1\times$ \\
Victim             &  2.6\% &  2.2\% & 0.16 & 0.12 & $1.3\times$ \\
Defense attorney   &  7.0\% &  8.5\% & 0.43 & 0.44 & $1.0\times$ \\
Prosecutor (self)  & 33.2\% & 41.9\% & 2.06 & 2.18 & $0.9\times$ \\
Politician         & 10.2\% &  6.8\% & 0.63 & 0.35 & $\mathbf{1.8\times}$ \\
Community member   & 11.8\% & 11.7\% & 0.73 & 0.61 & $1.2\times$ \\
Expert             &  8.1\% &  4.9\% & 0.50 & 0.26 & $\mathbf{1.9\times}$ \\
Journalist         &  6.7\% &  4.9\% & 0.41 & 0.25 & $1.6\times$ \\
Advocacy group     &  6.5\% &  3.9\% & 0.40 & 0.20 & $\mathbf{2.0\times}$ \\
\bottomrule
\end{tabular}
\end{table}

Coverage of progressive prosecutors features proportionally more expert sources ($1.9\times$), advocacy group voices ($2.0\times$), and politician quotes ($1.8\times$ per article). Police appear at roughly equal per-article rates; defense-attorney and self-quote proportions are also similar. These sourcing differentials suggest that progressive prosecutor coverage draws on a broader and more politically diverse source ecology. This is consistent with the accountability and reform framing patterns documented in Section~4, and with the ``toplash'' dynamics described by \citet{goldstein2024}: the elevated presence of expert, advocacy, and politician sources around progressive prosecutors is precisely the source ecology one would expect when institutional actors mobilize media relationships to contest reform agendas. The distribution of source \emph{stance} further illuminates the sourcing asymmetry:

\begin{table}[ht]
\centering
\caption{Source stance per article by prosecutor ideology (progressive $n = 3{,}423$; traditional $n = 1{,}340$).}
...
\midrule
Critical    & 1.64 & 1.11 & 0.19 & $< 10^{-8}$ \\
Supportive  & 1.35 & 0.78 & 0.27 & $< 10^{-15}$ \\
Neutral     & 3.20 & 3.31 & $-0.03$ & .300 \\
\bottomrule
\end{tabular}
\end{table}

Progressive prosecutors attract both more critical and more supportive sources per article, while neutral sources are equally represented, reflecting the greater political salience and controversy surrounding progressive prosecutors rather than a one-sided sourcing imbalance.

\textbf{Causal claims.} Progressive prosecutors face 2.3 times as many ``prosecutor caused harm'' attributions per article (0.43 vs.\ 0.19). ``Prosecutor helped'' attributions are also 1.5 times higher per article for progressives (0.14 vs.\ 0.09). The asymmetric harm ratio provides a specific structural mechanism through which progressive prosecutors receive more negative coverage.

\textbf{Claim types.} Per article, progressive prosecutors face more claims across all categories: performance (0.45 vs.\ 0.29, $1.6\times$), policy (0.48 vs.\ 0.28, $1.7\times$), character (0.42 vs.\ 0.35, $1.2\times$), and competence (0.31 vs.\ 0.25, $1.2\times$). The largest differentials appear for performance and policy criticisms, suggesting that progressive prosecutors are disproportionately criticized for \emph{what they do}, their decisions and policy choices, rather than for personal attributes, a pattern consistent with the accountability framing differential observed in Section~4.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{08_source_type_distribution.png}
  \caption{Source type distribution by prosecutor ideology from LLM structural extraction ($n = 4{,}763$ articles). Bars show proportions of total sources within each group; annotations show raw counts. The largest relative differentials are in expert sources ($1.9\times$), advocacy groups ($2.0\times$), and politicians ($1.8\times$ per article).}
  \label{fig:sourcetypes}
\end{figure}

\textbf{Per-prosecutor extraction density.} Pamela Price generates the most extraction-dense coverage (12.4 per article, $n = 436$), followed by Chesa Boudin (10.7, $n = 2{,}987$), Nancy O'Malley (9.1, $n = 302$), Brooke Jenkins (8.9, $n = 685$), and Steve Wagstaffe (7.1, $n = 353$).

\subsection*{Limitations}

The extraction sample is imbalanced (3,423 progressive vs.\ 1,340 traditional articles), reflecting the composition of the underlying corpus; all comparisons are expressed as per-article rates or proportions. Because articles were processed in each prosecutor's corpus order rather than drawn as a stratified random sample, the extracted subset may not be fully representative along temporal or event dimensions; a stratified re-extraction (by prosecutor, quarter, and outlet) would provide stronger assurance that the sourcing and claim-structure patterns generalize to the full corpus. The source type asymmetry, particularly the expert and advocacy-group disparities, suggests that tone differences detected in the main analysis may partly reflect differential journalistic sourcing practices rather than editorial bias per se.
