...
\begin{figure}[H]
  ...
  \caption{Effect size forest plot with 95\% bootstrap confidence intervals. The green band marks the negligible effect zone ($|d| < 0.2$). Results are grouped by analysis type: overall, paired-county, per-method, and theme attribution. Theme attribution ($d = -0.42$) is the largest effect plotted here, but it is substantially event-driven and reverses sign once recall- and crime-rising-flagged articles are removed, so the plotted magnitude is an upper bound (Section~\ref{sec:themes}).}
  \label{fig:forest}
\end{figure}
...
A trained research assistant, working independently of the author but not blind to the model, coded a stratified 100-article validation sample, labeling each article's stance toward the attributed prosecutor, its dominant media frame, and whether it was substantively about that prosecutor, and audited the corresponding extraction outputs. The validation program now comprises four completed packets---the 100-article stance-and-frame sample, a 160-extraction audit spanning 142 articles, a 120-article case-type coding task, and a 40-article extraction-recall audit. These 402 article-slots cover 373 distinct articles, because 23 articles were coded in two of the packets and 3 in all three; the first three packets alone cover 333 distinct articles, which is the denominator for the pooled relevance benchmark reported below. That non-blinding is a design caveat that applies throughout: every packet displayed the model's labels alongside the article text, so the coder formed her judgments with the model's answer in view. Non-blind coding can only inflate agreement, which makes the low agreement estimates reported below conservative in the direction that matters. A second, fully blinded coder, with an overlap set for inter-rater reliability, is in progress.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Researcher-authored theme attribution} (Section~\ref{sec:themes}), with hand-written compound patterns, hand-set method weights, and hand-set agreement multipliers: $d = +0.418$, the largest pooled effect in the paper.
\end{itemize}
