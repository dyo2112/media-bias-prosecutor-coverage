...
\begin{figure}[H]
  ...
  \caption{Effect size forest plot with 95\% bootstrap confidence intervals. The green band marks the negligible effect zone ($|d| < 0.2$). Results are grouped by analysis type: overall, paired-county, per-method, and theme attribution. Theme attribution ($d = -0.42$) is the largest raw effect among the pooled contrasts plotted here---though not the largest raw effect in the paper, which is the Alameda within-county theme contrast, $d = 0.68$ (Section~\ref{sec:themes})---and it is substantially event-driven, reversing sign once recall and crime-rising themes are removed; the plotted magnitude should be read as an upper bound.}
  \label{fig:forest}
\end{figure}
...
The mechanism is the one described in Section~\ref{sec:framing_methods}. The two instruments differ in level by roughly thirtyfold, and instrument assignment is correlated with group: $50.2\%$ of progressive-attributed articles are zero-shot-scored against $45.9\%$ of traditional-attributed ones. Progressive articles are thus disproportionately measured on the high-scale instrument, which lifts their pooled means for reasons that have nothing to do with what the coverage says. The pooled probability contrasts are reported here once, as the artifact they are, and carry no substantive claim anywhere in the paper. The honest summary is not that the pooled figures are conservative but that they are uninterpretable, and that the model-classified figures---which are the \emph{larger} ones on four of the five frames---are the ones to read.

Two cautions attach to the model-classified sample even in its primary role. First, its $6{,}175$ articles are, up to 29 articles, exactly the named-mention subsample used for the exclude-fallback composite sensitivity (Appendix~\ref{app:fallback_sensitivity}), for the mechanical reason given in Section~\ref{sec:framing_methods}: no named mention means no mention window, which forces the keyword frame fallback and the county-and-date attribution fallback together. The framing result and that composite sensitivity therefore do not independently corroborate one another; they report what one subsample looks like through two instruments. Second, and for the same reason, this subset is not a random half of the corpus but the half in which prosecutors are named, so part of the reason its differentials exceed the pooled ones reflects higher prosecutor centrality and not only cleaner frame measurement.
...
The robustness checks divide cleanly, and not all of them favor the reported estimates. The relevance- and period-based restrictions strengthen the effect: excluding fallback-attributed articles, restricting to prosecutor-focused articles, applying a stricter mention-dominance relevance rule, and restricting to tenure-period coverage all preserve or increase effect magnitudes, indicating attenuation rather than artifact in the full-sample estimates (Appendix~\ref{app:fallback_sensitivity}). These are overlapping rather than independent restrictions, and one pair is nearly identical: the exclude-fallback sample and the model-classified subset that now carries the primary framing analysis are the same 6{,}100-odd articles up to 29 cases (Jaccard $= 0.996$; Section~\ref{sec:framing_methods}), so the framing result and the composite fallback check should be counted as one piece of evidence, not two. The pooled estimate is not, however, robust to the composition of the traditional baseline. Excluding Brooke Jenkins raises the composite differential to $d = -0.233$, but excluding Steve Wagstaffe, the sole San Mateo County prosecutor, halves it to $d = -0.070$; across all five leave-one-prosecutor-out fits the composite ranges from $d = -0.233$ to $d = -0.070$ (Appendix~\ref{app:baseline_sensitivity}). Reporting only the leave-Jenkins-out check, as an earlier version of this paper did, gave a one-sided picture of that sensitivity.
...
\item \textbf{Extraction recall is the weak side of the ledger, and what it misses is systematic.} The recall audit puts the false-negative rate for source attributions at 28--34\% and for causal claims at 31--39\% (Section~\ref{sec:human_validation}). Missed items are dominated by unnamed and documentary attribution---``authorities,'' ``investigators,'' court and jail records, testimony, filings---while named, directly quoted individuals are reliably captured. The absolute source-composition shares in Appendix~\ref{app:extraction} therefore understate institutional and documentary sourcing, and the chi-square on the overall source-type distribution should not be read as a claim about true composition. Between-group ratios are the more robust quantity: recall failure shows no detectable asymmetry across prosecutor types (1.55 versus 2.45 missed sources per article, Welch $p = .310$), though at 20 articles per group that symmetry test is badly underpowered. Attribute-level precision is likewise the weakest layer of the extraction (72.4\% overall, 62.5\% for causal claims), so attribute-based ratios such as harm attribution and source stance carry materially more noise than the class-level counts.

\item \textbf{The framing fallback and the attribution fallback are one subsample, not two.} The keyword frame fallback and the county-and-date attribution fallback flag almost exactly the same articles (Jaccard $= 0.996$; 6{,}637 of 6{,}651; Section~\ref{sec:framing_methods}), because both fire on the same condition: no named prosecutor mention, hence no mention window. The primary (zero-shot-only) framing analysis and the exclude-fallback composite sensitivity are consequently two instruments applied to one subsample, and the paper does not treat their agreement as independent corroboration. It also means that roughly half of the analysis corpus is measured by dictionary rather than by model on the framing layer, and that the same half is attributed by county-and-date rather than by name. A direct consequence is that the primary framing estimates rest on half the corpus, and on the half in which prosecutors are named, so they are better measured but not representative of the whole attributed sample.

\item \textbf{The two frame instruments are not on a common scale, and pooling them produces figures that are not measurements.} The zero-shot classifier returns entailment probabilities and the keyword fallback returns dictionary match fractions; mean levels differ by roughly a factor of thirty (e.g., human interest $0.830$/$0.904$ under the classifier against $0.0256$/$0.0261$ under the fallback). Instrument assignment is correlated with group ($50.2\%$ of progressive-attributed articles are zero-shot-scored against $45.9\%$ of traditional-attributed ones), so pooled means partly track which instrument scored an article. The evidence that this is fatal rather than merely noisy is that for two of the five frames the pooled Cohen's $d$ lies outside the range spanned by its own two subgroups (conflict $+0.074$ against $-0.119$ and $+0.035$; human interest $+0.001$ against $-0.413$ and $-0.008$; Table~\ref{tab:instrument_mixing}), which no weighted combination on a common scale can do. The continuous frame contrasts are therefore estimated on the model-classified subset only, and the pooled probability contrasts reported in an earlier version of this paper are withdrawn. An earlier version also described the pooled framing figures as conservative, on the theory that the fallback attenuated the differential; that explanation cannot be right, since attenuation cannot move an estimate outside the range of its parts. The dominant-frame chi-square is less exposed, because argmax within an article is in-kind across instruments, but the fallback's argmax runs over near-zero values, so the pooled chi-square is reported as secondary to the model-classified one.
...
\item \textbf{The corpus is one metropolitan area, and it is not purely local.} Whether these patterns generalize to reform prosecutors elsewhere (Krasner in Philadelphia, Foxx in Chicago, Gasc\'{o}n in Los Angeles) is an open empirical question. Within the corpus itself, two of the 21 publications are national rather than Bay Area outlets---nytimes.com and politico.com, together 5{,}223 articles (3.8\% of the corpus) and 395 of the 12{,}827 attributed articles (3.1\%; Section~\ref{sec:data}). Their coverage is selected on national newsworthiness in a way local coverage is not, and the imbalance this produces is visible: 306 of those 395 articles (77\%) are attributed to a progressive prosecutor, against 51\% in the analysis sample as a whole, and 292 of them concern Boudin. At 3.1\% of the analysis sample the two outlets cannot drive the reported contrasts, but they tilt in the direction of the reported effect rather than against it, and the paper's framing as a study of \emph{Bay Area} coverage is an approximation rather than an exact description of the corpus.
...
\centering
\small
\caption{Illustrative examples of each dominant media frame (Appendix C). Each row is an article whose \texttt{dominant\_frame} is the frame named in the first column; the score is that article's probability on that frame, from \texttt{output/05\_frames.parquet}. All five are zero-shot-classified rather than keyword-fallback articles.}
...
On the model-classified sample that carries the primary framing analysis, progressive prosecutors are more than twice as likely to receive accountability framing (40.4\% vs.\ 18.7\%) and more than four times as likely to receive reform framing (5.0\% vs.\ 1.2\%), while traditional prosecutors receive substantially more human-interest coverage (32.3\% vs.\ 17.4\%; see also Figure~\ref{fig:heatmap} and its caption caveat about pooling the two frame instruments).
...
Read against the within-county estimates, this is a coherence gain rather than a loss. Dropping Wagstaffe from the pooled contrast ($d = -0.070$), the San Francisco pair ($d = -0.068$, $p = .0020$), and the Alameda pair ($d = -0.069$, $p = .138$) are three routes to the composite differential that avoid cross-county comparison, and all three land at $d \approx -0.07$. The pooled $d = -0.151$ is roughly double, and the excess is attributable to cross-county heterogeneity in coverage levels. Since the paper treats the within-county paired design as its strongest identification, the pooled composite is best read as an upper bound.
...
Before the estimates: this exclusion does not define a new subsample. The articles retained here are, up to 29 cases out of 12{,}827, the same articles the framing layer classifies with the zero-shot model rather than the keyword fallback (Jaccard $= 0.996$ between the two indicators; Section~\ref{sec:framing_methods}). Both flags fire on the same underlying condition---no named prosecutor mention, hence no mention window for either the frame classifier or name-based attribution. The exclude-fallback composite sensitivity below and the model-classified framing analysis in Section~\ref{sec:framing_results} are therefore the same subsample viewed through two instruments, and their agreement should be read as one fact about that subsample rather than as two independent confirmations.
