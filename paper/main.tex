...
Two cautions attach to this subset. First, the 6{,}175 model-classified articles are, up to 29 articles, exactly the named-mention subsample used for the exclude-fallback composite sensitivity (Appendix~\ref{app:fallback_sensitivity}), for the mechanical reason given in Section~\ref{sec:framing_methods}: no named mention means no mention window, which forces the keyword frame fallback and the county-and-date attribution fallback together. The two checks therefore do not independently corroborate one another; they report what one subsample looks like through two instruments. Second, and for the same reason, this subset is not a random half of the corpus but the half in which prosecutors are named, so part of the strengthening reported here reflects higher prosecutor centrality rather than better frame measurement alone.
...
\item \textbf{The framing fallback and the attribution fallback are one subsample, not two.} The keyword frame fallback and the county-and-date attribution fallback flag almost exactly the same articles (Jaccard $= 0.996$; 6{,}637 of 6{,}651; Section~\ref{sec:framing_methods}), because both fire on the same condition: no named prosecutor mention, hence no mention window. The zero-shot-only framing sensitivity and the exclude-fallback composite sensitivity are consequently two instruments applied to one subsample, and the paper does not treat their agreement as independent corroboration. It also means that roughly half of the analysis corpus is measured by dictionary rather than by model on the framing layer, and that the same half is attributed by county-and-date rather than by name.
